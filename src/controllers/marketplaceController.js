...
const { Product, User, Notification, ContactSubmission, sequelize } = require('../models');
...
exports.getPublicProducts = async (req, res) => {
  try {
    ...
    const { count, rows: products } = await Product.findAndCountAll({
      ...
      order: orderBy,
      attributes: ['id', 'name', 'description', 'price', 'category', 'stock', 'image_url', 'specifications', 'views', 'created_at']
    });
...
exports.getPublicProductById = async (req, res) => {
  try {
    ...
    const product = await Product.findByPk(id, {
      attributes: ['id', 'name', 'description', 'price', 'category', 'stock', 'image_url', 'specifications', 'views', 'is_active', 'created_at', 'updated_at']
    });
...
exports.submitContact = async (req, res) => {
  try {
    ...
    const adminUsers = await User.findAll({
      where: {
        role: ['super_admin', 'admin', 'national_commissioner'],
        status: 'active'
      },
      attributes: ['id']
    });
...
exports.createProduct = async (req, res) => {
  try {
    const { name, description, price, category, stock, specifications, is_active } = req.body;
    ...
    if (req.user) {
      await Notification.create({
        ...
        message: `Product "${name}" has been added to the marketplace.`,
  ...
  try {
    ...
    if (req.user) {
      await Notification.create({
        ...
        message: `Product "${oldName}" has been updated.`,
        type: 'product_management',
        priority: 'low',
        is_read: false
      });
  ...
  try {
    ...
    if (req.user) {
      await Notification.create({
        ...
        message: `Image for "${product.name}" has been updated.`,
        type: 'product_management',
        priority: 'low',
        is_read: false
      });
  ...
  try {
    ...
    if (req.user) {
      await Notification.create({
        ...
        message: `${ids.length} products (${productNames}) have been deleted.`,
        type: 'product_management',
        priority: 'medium',
        is_read: false
      });
  ...
  try {
    ...
    if (req.user) {
      await Notification.create({
        user_id: req.user.id,
        title: '📊 Stock Updated',
        message: `Updated stock for ${successful} products.`,
        type: 'product_management',
        priority: 'low',
        is_read: false
      });
...
exports.getMessages = async (req, res) => {
  try {
    ...
    const { count, rows: messages } = await ContactSubmission.findAndCountAll({
      where: whereCondition,
      limit: parseInt(limit),
      offset: parseInt(offset),
...
exports.getMessageById = async (req, res) => {
  try {
    ...
    const message = await ContactSubmission.findByPk(id, {
      include: [
        {
          model: User,
          as: 'replier',
